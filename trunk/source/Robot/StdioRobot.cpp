...
void StdioRobot::render(Graphics* g)
{
	...
	translationMatrix.SetTranslation(mapX + (_currentColumn+0.5f)*cellWidth, mapY + (_currentRow+0.5f)*cellWidth);

	g->pushMatrix(rotationMatrix);
	g->pushMatrix(translationMatrix);
	g->drawImage(_bodyTexture, Rect(-(cellWidth/2.0f), -(cellWidth/2.0f), cellWidth, cellWidth));
	g->popMatrix();
}
